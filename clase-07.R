...
df = mutate(.data = df , height_cm = height*2.54,
            weight_hcm = weight/height_cm) #generar las variables

df = arrange(.data=df , desc(height_cm))

head(x=df , n=5)

## Otra forma de hacerlo es emplear el operador pipe `%>%`:  pipe ayuda a ahorrar escritura de código

df = as_tibble(x = women) %>% 
  
  mutate(height_cm = height*2.54, weight_hcm = weight/height_cm) %>% #se borro df porque ya entiende que se trabaja con el mismo
  arrange(desc(height_cm)) #organizar variables
...
df <- import("https://www.datos.gov.co/resource/epsv-yhtj.csv") %>%
as_tibble() %>%
select(-cod_ase_) %>% #el menos es para dejar todas menos esto 
...
db<- bind_cols(db_1,db_2) #pegar los dos vectores: no es útil para combinar bases de datos si no estoy seguro que están en el orden
#para que no se una mal se usa join
...
#### **Ejemplo: left_join()** #usar las llaves como algo único
left<- left_join (x=data_1, y=data_2, by=c("Casa", "Visita"))

#### **Ejemplo: right_join()**
right<- right_join(x=data_1,y=data_2, by=c("Casa", "Visita"))

#### **Ejemplo: inner_join()**
inner<- inner_join(x=data_1, y=data_2
                   
#### **Ejemplo: full_join()**
full1<- full_join(x=data_1, y=data_2, by=c("Casa", "Visita"))
full2<- full_join(x=data_1, y=data_2, by=c("Casa")) #no se debe hacer porque aparece 2 veces
...
##intentar unir
df_4 <- full_join(x=data_1, y=data_2, by=c("Casa", "Visita"))
